...
#Chek if this is a 64-bit powershell
if ([IntPtr]::size -ne 8)
{
    Write-Host "This is not a 64-bit powershell! please run powershell from C:\Windows\system32\WindowsPowerShell\v1.0\powershell.exe"
    exit 0
}
...
{
    write-host "Trying to use this machine [$ControllerName] as HLK  Controller"
}

# Connect to server (controller)
write-host Connecting to $ControllerName ...
$Manager = new-object -typename Microsoft.Windows.Kits.Hardware.ObjectModel.DBConnection.DatabaseProjectManager -Args $ControllerName
...
$RootPool.GetChildPools() | foreach {
    # Ignore "Archive" pool
    if($_.Name -eq "Archive")
    {
        return # This return behaves like continue inside this foreach loop!
    }
...
if( $AmountOfMachinesInPool -gt "1")
{
    ...
    $ClientServerMachine = $TestPool.GetMachines()[0]
}
else
{
    $ClientMachine = $null
    ...
    )
    {
        if ( $_.Name.Contains("[2 Machine]") )
        {
            if( $ClientMachine -ne $null)
            {
                ...
                Try
                {
                    $Role.Roles[0].AddMachine($ClientServerMachine)
                }
                Catch
                {
